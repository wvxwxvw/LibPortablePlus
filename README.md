...
```csharp
    ...
    NewPC.bat
    	Можно запустить перед использованием на новом ПК или после изменения
    	пути сборки, и после этого два раза перезапустить бразер.
    	Сам я с проблемой неподхвата профиля на новом ПК не сталкивался, текущая
    	и предыдущая сборки для меня работают на новых ПК без удаления кеша.
    	Разве что - при первом запуске могут появится уведомление о том что
    	добавлено расширение или языковой пакет (если установлен второй).
    	Могу лишь предположить, что проблемы появляются у тех кто изменяет в
    	сборке конфигурационные файлы или не пользуется скриптом очистки.
    	При проверке этого поведения, в пределах одного ПК, не забывайте удалять
    	сборку в старом расположении, иначе расширения могут подхватиться по
    	старым путям, и ни в коем случае не удаляйте extensions.json перед
    	запуском в новом расположении - сломаете профиль.
    VACUUM.bat
    	...
    	Полезно перед бэкапом, дабы бестолково не раздувать архивы или
    	исключить сохранение конфиденциальной информации
```
...
соблюдая выбранную цветовую гамму, помимо прочего ликвидирует все недостатки Proton  
и других "улучшений" Firefox, типа мегабара и невменяемого размера менюшек,  
легко редактируется под ваши предпочтения  
